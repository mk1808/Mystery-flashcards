...
export async function POST(request: NextRequest) {
    ...
    const existingUser = { login: "login", password: "$2b$10$GbDHZZ/gqafT/llNQFWsX.E4EfKeG.RWkBxXSt1k4W7erU6v61TDK" }

    if (!existingUser || !checkPasswordMatch(loginForm.password, existingUser.password)) {
        return new Response('Invalid credentials!', { status: 401 });
    }

    const token = await signToken({ login: existingUser.login });
